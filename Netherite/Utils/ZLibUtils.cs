...
namespace Netherite.Utils
{
    ...
    public static class GZipUtils
    {
        public static async Task CompressAsync(byte[] byteIn, Stream outStream)
        {
            using var stream = new GZipStream(new MemoryStream(), CompressionMode.Compress, CompressionLevel.BestCompression);
        ...
        public static async Task CompressAsync(Stream inputStream, Stream outputStream)
        {
            await using var stream = new GZipStream(inputStream, CompressionMode.Compress, CompressionLevel.BestCompression);
            await stream.CopyToAsync(outputStream);
        }

        public static async Task<byte[]> DecompressAsync(byte[] byteIn, int size)
        {
            using var stream = new GZipStream(new MemoryStream(byteIn, false), CompressionMode.Decompress, CompressionLevel.BestSpeed);
